...
def get_signal(df, lookback=7):
    ...
    def signal(row):
        ...
        long = (
            ...
            #
            and check_tsi
            and row.sar_long < row.ha_close
            # ------------------------------- 8h timeframe ------------------------------- #
            # and row.sar_long_8h < row.close_8h
        )
...
async def get_signals_crypto_long_live1(symbols=[], data_length=1000, isEnabled=True, timeframe="6h", datetime_start=None, datetime_end=None, lookback=7):
    if isEnabled == False:
        return []

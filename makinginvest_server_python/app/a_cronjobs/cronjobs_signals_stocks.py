...
@aiocron.crontab("*/15 10-23 * * 1-5")
async def cron_signals_stocks_macd_momemtum_long_v1():
    if is_production == "True" and is_allow_cron == "True" and is_data_mode != "True":
        app_logger().error("started signals-stocks-all-live1")
        await asyncio.sleep(120)
        start = time.time()
        try:
            async with aiohttp.ClientSession() as session:
                async with session.patch(f"{BASE_URL}/signals-stocks-all-live1?apikey={apikey}", timeout=1200) as response:
                    await response.json()
                    stat = f"signals-stocks-all-live1: {(time.time() - start) / 60:.2f} minutes"
                    app_logger().info(stat)

        except HTTPException as e:
            app_logger().info("signals-stocks-all-live1: ", e)
            raise e

        except Exception as e:
            app_logger().error("signals-stocks-all-live1", e)

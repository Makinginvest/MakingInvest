...
async def get_closed_signals_results_api_v0_v1(signalsCollection: str = None, name: str = None, nameVersion: str = None):
    print(signalsCollection, name, nameVersion)
    try:
        collection = database_mongodb_client[signalsCollection]
        seven_days_ago = datetime.utcnow() - timedelta(days=7)
        thirty_days_ago = datetime.utcnow() - timedelta(days=30)
        data30days = await collection.find(
            {
                "name": name,
                "nameVersion": nameVersion,
                "entryDateTimeUtc": {"$gte": thirty_days_ago},
            }
        ).to_list(length=None)

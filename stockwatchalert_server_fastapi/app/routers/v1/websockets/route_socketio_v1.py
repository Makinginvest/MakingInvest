...
from app._firebase.a_validate_key_jsonwebtoken import validate_apikey_json_webtoken_bool
...
sio_v1 = socketio.AsyncServer(async_mode="asgi", cors_allowed_origins="*")
sio_app_v1 = socketio.ASGIApp(socketio_server=sio_v1)
...
async def connect(sid, data):
    ...
    if is_token_valid == False:
        ...
        return False
    if not int(appBuildNumber) >= 1000:
        app_logger().error(f"token not valid {jsonWebToken} {apikey} {appBuildNumber}")
        return False
    ...
    await sio_v1.emit("connect", f"connected {sid} ", room=sid)
...
async def start_background_tasks_crypto_prices(manager: ConnectionManagerSockerIo, sid=None):
    if len(manager.prices_crypto) > 0:
        await sio_v1.emit("prices_crypto", {"data": manager.prices_crypto}, room=sid)
    ...
    if not manager.is_background_task_running_crypto and is_websocket_mode == "True":
        while True:
            ...
            try:
                ...
                if len(data) > 0:
                    await sio_v1.emit("prices_crypto", {"data": data})
                    manager.set_is_background_task_running_crypto(True)
...
async def start_background_tasks_forex_prices(manager: ConnectionManagerSockerIo, sid=None):
    if len(manager.prices_forex) > 0:
        await sio_v1.emit("prices_forex", {"data": manager.prices_forex}, room=sid)
    ...
    if not manager.is_background_task_running_forex and is_websocket_mode == "True":
        while True:
            ...
            try:
                ...
                if len(data) > 0:
                    await sio_v1.emit("prices_forex", {"data": data})
                    manager.set_is_background_task_running_forex(True)
...
async def start_background_tasks_stocks_prices(manager: ConnectionManagerSockerIo, sid=None):
    if len(manager.prices_stocks) > 0:
        await sio_v1.emit("prices_stocks", {"data": manager.prices_stocks}, room=sid)
    ...
    if not manager.is_background_task_running_stocks and is_websocket_mode == "True":
        while True:
            ...
            try:
                ...
                if len(data) > 0:
                    await sio_v1.emit("prices_stocks", {"data": data})
                    manager.set_is_background_task_running_stocks(True)
...
async def start_background_tasks_market_analysis(manager: ConnectionManagerSockerIo, sid=None):
    if manager.market_analysis is not None and manager.market_analysis.get("cryptoSymbolsAnalysis") is not None:
        ...
        await sio_v1.emit("market_analysis", {"data": data}, room=sid)
        ...
        while True:
            ...
            try:
                ...
                if data is not None and data["dtUpdated"] != manager.dt_market_analysis_updated:
                    ...
                    await sio_v1.emit("market_analysis", {"data": data})
...
async def start_background_tasks_symbols_tracker(manager: ConnectionManagerSockerIo, sid=None):
    if manager.symbols_tracker is not None:
        ...
        await sio_v1.emit("symbols_tracker", {"data": data}, room=sid)
        ...
        while True:
            ...
            try:
                ...
                if data is not None and data["dtUpdated"] != manager.dt_symbols_tracker_updated:
                    ...
                    data = convert_bson_json(data)
                    await sio_v1.emit("symbols_tracker", {"data": data})
...
async def start_background_tasks_news_aggr(manager: ConnectionManagerSockerIo, sid=None):
    if manager.news_aggr is not None:
        ...
        await sio_v1.emit("news_aggr", {"data": data}, room=sid)
        ...
        while True:
            ...
            try:
                ...
                if data is not None and data["dtNewsAggrUpdated"] != manager.dt_news_aggr_updated:
                    ...
                    data = convert_bson_json(data)
                    await sio_v1.emit("news_aggr", {"data": data})
...
async def start_background_signal_aggr_open_mongodb(manager: ConnectionManagerSockerIo, sid=None):
    if len(manager.signal_aggr_open) > 0:
        await sio_v1.emit("signal_aggr_open_v1", {"data": manager.signal_aggr_open}, room=sid)

    if not manager.is_background_task_running_signal_aggr_open and is_websocket_mode == "True":
        while True:
            try:
                (run_update, signalsAggrOpenUpdatedDateTime) = await get_signals_aggr_open_mongodb_lastest_change(
                    manager.dt_signal_aggr_open_updated,
                    manager.signal_aggr_open,
                )

                if run_update:
                    manager.set_dt_signal_aggr_open_updated(signalsAggrOpenUpdatedDateTime)
                    data = await get_signals_aggr_open_mongodb()
                    data = convert_bson_json(data)
                    manager.set_signal_aggr_open(data)
                    await sio_v1.emit("signal_aggr_open_v1", {"data": data})

                await asyncio.sleep(10)

            except Exception as e:
                manager.set_is_background_task_running_signal_aggr_open(False)
                manager.set_dt_signal_aggr_open_updated(None)
...
async def get_signals_aggr_open_mongodb():
    try:
        collection = database_mongodb_client["signalsAggrOpenV1"]
...
async def get_signals_aggr_open_mongodb_lastest_change(signalsAggrOpenUpdatedDateTime: datetime, signalsAggr: List):
    try:
        collection = database_mongodb_client["appControlsPrivateV1"]

        if signalsAggrOpenUpdatedDateTime is None:
            query = await collection.find({"nameId": "appControlsPrivateV1"}).limit(1).to_list(length=1)
            if query:
                return True, query[0].get("signalsAggrOpenUpdatedDateTime")

            return (False, signalsAggrOpenUpdatedDateTime)

        query = await collection.find({"nameId": "appControlsPrivateV1", "signalsAggrOpenUpdatedDateTime": {"$gt": signalsAggrOpenUpdatedDateTime}}).limit(1).to_list(length=1)
        if query:
            return True, query[0].get("signalsAggrOpenUpdatedDateTime")

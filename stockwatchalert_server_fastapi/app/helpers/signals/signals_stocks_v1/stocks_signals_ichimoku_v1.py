...
async def get_stocks_signals_ichimoku_v1():
    ...
    nameId = "signals_stocks_v1"
    nameIsActive = True
    nameIsAdminOnly = False
    nameMarket = "stocks"
    nameNotificationTitle = "Stocks - Scalper"
    nameSort = 1
    nameType = "Stocks"
    ...
    datetime_start = datetime.utcnow() - pd.Timedelta(days=35 * 5 * 1)
    ...
    if gen_new_signals:
        ...
        # print(df_timeframe.groupby("symbol").tail(26)[["symbol", "dateTimeUtc_{timeframe_inf}", "dateTimeUtc", "open", "close", "high", "low", "open_{timeframe_inf}", "close_{timeframe_inf}", "high_{timeframe_inf}", "low_{timeframe_inf}"]])
...
def calculate_signals(dataframe: pd.DataFrame, timeframe_inf: str):
    dataframe.loc[
        (
            ...
            # ----------------------------------- TEST ----------------------------------- #
            & (dataframe[f"is_top_rank_{timeframe_inf}"] == True)
            # & (dataframe[f"long_emas_stacked_{timeframe_inf}"] == True)
            & (dataframe[f"ewo_{timeframe_inf}"] > 2.25)
        ),
        ["entryType", "enter_tag"],
    ...
    dataframe.loc[
        (
            ...
            # ----------------------------------- TEST ----------------------------------- #
            & (dataframe[f"is_top_rank_{timeframe_inf}"] == True)
            # & (dataframe[f"short_emas_stacked_{timeframe_inf}"] == True)
            & (dataframe[f"ewo_{timeframe_inf}"] < -2.25)
        ),
        ["entryType", "enter_tag"],

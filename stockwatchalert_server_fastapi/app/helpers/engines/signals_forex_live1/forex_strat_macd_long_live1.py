...
def get_signal(df, lookback=7):
    ...
    df["rsi_prev_rolling_87_check"] = df["rsi_prev"].rolling(lookback * 1).apply(lambda x: all(i < 87 for i in x))
    ...
    def signal(row):
        ...
        long = (
            ...
            and row.rsi_prev_rolling_87_check
            ...
            and row.high > row.ichimoku_span_a
            and row.high > row.ichimoku_span_b
...
async def get_signals_forex_long_live1(symbols=[], data_length=1000, isEnabled=True, timeframe="6h", datetime_start=None, datetime_end=None, lookback=7):
    if isEnabled == False:
        return []

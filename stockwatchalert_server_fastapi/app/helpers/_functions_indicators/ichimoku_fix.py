...
def ichimoku_fix(high, low, close, tenkan=None, kijun=None, senkou=None, include_chikou=True, offset=None, **kwargs):
    ...
    _span_a = span_a[-kijun:].shift(-1).copy()  # Added the -1 to include 0.
    _span_b = span_b[-kijun:].shift(-1).copy()  # Added the -1 to include 0.

    span_a = span_a.shift(kijun - 1)  # Added the -1 to include 0.
    span_b = span_b.shift(kijun - 1)  # Added the -1 to include 0.
    chikou_span = close.shift(-kijun + 1)  # Added the 1 to shift it 25.
...
ichimoku_fix.__doc__ = """Ichimoku Kinkō Hyō (ichimoku)

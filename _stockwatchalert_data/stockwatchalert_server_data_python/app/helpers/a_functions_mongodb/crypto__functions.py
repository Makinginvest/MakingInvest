...
def get_crypto_symbols_with_futures(path="_datasets/data/_data_symbols_crypto_futures.csv"):
    symbols = pd.read_csv(path)
    symbols = symbols["symbol"].tolist()
    # remove /
...
async def get_USDT_symbols_by_value_by_df(path="_datasets/data/_data_symbols_crypto_usdt_busd.csv"):
    symbols = pd.read_csv(path)

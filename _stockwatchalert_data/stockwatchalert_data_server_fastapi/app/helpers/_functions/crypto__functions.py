...
async def get_USDT_symbols_binance():
...
def get_crypto_symbols_with_futures():
    symbols = pd.read_csv("_project/datasets/data/_data_symbols_crypto_futures.csv")
    symbols = symbols["symbol"].tolist()
    symbols = [s.replace("/", "") for s in symbols]
    return symbols
...
async def get_USDT_symbols_by_value_by_df():
    symbols = pd.read_csv("_project/datasets/data/_data_symbols_crypto_usdt_busd.csv")

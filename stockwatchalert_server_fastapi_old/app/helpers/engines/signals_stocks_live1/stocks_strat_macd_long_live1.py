...
def get_signal(df, lookback=7):
    ...
    def signal(row):
        ...
        long_gap_check = abs(row.r_close_prev - row.r_open) / row.r_close_prev < 0.02
...
async def get_signals_stocks_long_live1(symbols=[], data_length=1000, isEnabled=True, timeframe="6h", datetime_start=None, datetime_end=None, lookback=7):
    if isEnabled == False:
        return []

...
async def connect(sid, data):
    ...
    asyncio.create_task(start_background_tasks_market_activities_aggr(manager=manager, sid=sid))
...
async def start_background_tasks_market_activities_aggr(manager: ConnectionManagerSockerIo, sid=None):
    if manager.market_activities_aggr is not None:
        data = convert_bson_json(manager.market_activities_aggr)
        await sio.emit("market_activities_aggr", {"data": data}, room=sid)

    if not manager.is_background_task_running_market_activities_aggr and is_websocket_mode == "True":
        while True:
            manager.set_is_background_task_running_market_activities_aggr(True)
            try:
                collection = database_mongodb_data["marketActivities"]
                data = await collection.find_one({"type": "all"})
                #

                if data is not None and data["lastUpdatedDateTimeUtc"] != manager.last_updated_datetime_utc_market_activities_aggr:
                    manager.set_last_updated_datetime_utc_market_activities_aggr(data["lastUpdatedDateTimeUtc"])
                    manager.set_market_activities_aggr(data)

                    data = convert_bson_json(data)
                    await sio.emit("market_activities_aggr", {"data": data})

                await asyncio.sleep(30)

            except Exception as e:
                print(e)
                manager.set_is_background_task_running_market_activities_aggr(False)
                manager.set_market_activities_aggr(None)
                app_logger.error(e)
                await asyncio.sleep(30)

...
async def get_binance_data_monthly_zip(session, symbol=None, interval=None, year=None, month=None):
    try:
        ...
        async with session.get(url) as resp:
            ...
            return f"_datasets/_temp/binance_raw/{symbol}-{interval}-{year}-{month}.csv"
...
async def get_binance_data_monthly_by_symbol(symbol, timeframe, lookback_months=1):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if not os.path.exists(f"_datasets/_binance/{timeframe}"):
                os.makedirs(f"_datasets/_binance/{timeframe}")

            df.to_csv(f"_datasets/_binance/{timeframe}/{symbol}-{timeframe}.csv", index=False)
...
async def get_binance_data_daily_zip(session, symbol=None, interval=None, year=None, month=None, day=None):
    try:
        ...
        async with session.get(url) as resp:
            ...
            return f"_datasets/_temp/binance_raw/{symbol}-{interval}-{year}-{month}-{day}.csv"
    except Exception as e:
        return None
...
async def get_binance_data_daily_by_symbol(symbol, timeframe="15m", lookback_days=15):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        df["time"] = pd.to_datetime(df["time"], unit="ms")
        if os.path.exists(f"_datasets/_binance/{timeframe}/{symbol}-{timeframe}.csv"):
            df_existing = pd.read_csv(f"_datasets/_binance/{timeframe}/{symbol}-{timeframe}.csv")
        ...
        df.to_csv(f"_datasets/_binance/{timeframe}/{symbol}-{timeframe}.csv", index=False)
        os.remove(f"_datasets/_binance/{timeframe}/{symbol}-{timeframe}.csv")
        ...
        if timeframe == "15m":
            ...
            tasks = [
                ...
                update_mongodb_data_by_symbol(df_3h.to_dict("records"), baseCollection="historicalCrypto", timeframe="3h"),
...
async def update_ohlcv_data_mongodb_recent(symbol, timeframe="15m", limit=500, exchange=None) -> pd.DataFrame:
    try:
        ...
        if timeframe == "5m":
            df_5m = df
            tasks = [update_mongodb_data_by_symbol(df_5m.to_dict("records"), baseCollection="historicalCrypto", timeframe="5m")]
            await asyncio.gather(*tasks)
...
async def crypto_update_all_mongodb_historical_recent(timeframe="15m", limit=200, path="_datasets/data/_data_symbols_crypto_usdt_busd.csv"):
    try:
        start = time.time()
        ...
        for i in range(0, len(symbols), 15):
            ...
            val = await asyncio.gather(*[update_ohlcv_data_mongodb_recent(symbol, timeframe=timeframe, limit=limit, exchange=exchange) for symbol in symbols_batch])
            # append each dataframe to df_all
